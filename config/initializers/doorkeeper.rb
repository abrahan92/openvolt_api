...
require 'custom_token_extractor'
...
Doorkeeper.configure do
  ...
  #   # if user && user&.authenticate_platform_access?(params[:request_from])
  #   #   user
  #   # else
  #   #   nil
  #   # end
  ...
  resource_owner_from_assertion do
    if server.client && params[:provider] && params[:assertion]
      ...
      when "google"
        ...
        payload = validator.check(params[:assertion], Settings.providers.google.client_id)
  ...
  #
  default_scopes  :read
  optional_scopes "credentials:me"
  ...
  #   request.cookies['defaultapi_access_token']
  # }

  access_token_methods CustomTokenExtractor
...
Doorkeeper::JWT.configure do
  ...
  token_payload do |opts|
    ...
    profile =  if user.properties[:account_type] == 'other'
      user.try(:other)
    elsif user.properties[:account_type] == 'new_other'
      user.try(:new_other)
    else
      user.try(:admin)
    end

    {
      ...
      user: User::Representer::DoorkeeperUser.new(user)
    }
  end
